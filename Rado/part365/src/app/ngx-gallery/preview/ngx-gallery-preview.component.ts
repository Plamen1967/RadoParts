...
@Component({
    standalone: true,
    selector: 'app-ngx-gallery-preview',
    templateUrl: './ngx-gallery-preview.component.html',
    styleUrls: ['./ngx-gallery-preview.component.scss'],
    imports: [NgxGalleryActionComponent, NgxGalleryBulletsComponent, NgxGalleryArrowsComponent]
})
export class NgxGalleryPreviewComponent implements OnInit, OnChanges {
    ...
    private getClientX(e: MouseEvent | TouchEvent): number {
        if (e instanceof TouchEvent) {
            return e.touches && e.touches.length ? e.touches[0].clientX : 0;
        } else {
            return e.clientX;
        }
    }
    ...
    private getClientY(e: MouseEvent | TouchEvent): number {
        if (e instanceof TouchEvent) {
            return e.touches && e.touches.length ? e.touches[0].clientY : 0;
        } else {
            return e.clientY;
        }
    }
    ...
    private isKeyboardNext(e: KeyboardEvent): boolean {
        return e.key === "ArrowRight" ? true : false;
    }

    private isKeyboardPrev(e: KeyboardEvent): boolean {
        return e.key === "ArrowLeft" ? true : false;
    }

    private isKeyboardEsc(e: KeyboardEvent): boolean {
        return e.key === "Escape" ? true : false;
    }
    ...
    private _show() {
        ...
        setTimeout(() => {
            if (this.isLoaded(this.previewImage?.nativeElement)) {
                ...
                if (this.previewImage) this.previewImage.nativeElement.onload = () => {
                    this.loading = false;
