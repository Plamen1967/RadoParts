...
export class AuthenticationService {
    ...
    login(userName: string, password: string) {
        return this.http.post<AuthenticatedUser>(`${environment.restAPI}/users/autenticate`, { userName, password }, this.httpHeader).pipe(
            map((user) => {
    ...
    updateUser(user: User) {
        return this.http.post<User>(`${environment.restAPI}/users/updateUser`, user).pipe()
    }

    validateToken() {
        const headers = { 'content-type': 'application/json' }
        return this.http.post<User>(`${environment.restAPI}/users/ValidateJWT`, `"${this.currentUserValue?.token}"`, { headers }).pipe(first())
    }

    updatePassword(oldPassword_: string, password_: string) {
        return this.http.post(`${environment.restAPI}/users/updatePassword`, { oldPassword: oldPassword_, password: password_ }, this.httpHeader).pipe(first())
    }
