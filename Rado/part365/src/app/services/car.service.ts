...
export class CarService {
    ...
    fetch(modelId: number) {
        ...
        return this.http.get<Car[]>(`${environment.restAPI}/car/GetCarsByModel`, { params }).pipe(
            first(),
    ...
    fetchCarNameId(bus: number): Observable<CarNameId[]> {
        ...
        return this.http.get<CarNameId[]>(`${environment.restAPI}/car/GetCarNameId`, { params }).pipe(
            first(),
    ...
    fetchBusNameId(): Observable<CarNameId[]> {
        return this.http.get<CarNameId[]>(`${environment.restAPI}/car/GetCarNameId`).pipe(
            first(),
            tap(() => this.loggerService.log('fetch cars GetCarNameId')),
            catchError(this.handleError('fetch cars GetCarNameId', []))
        )
    ...
    fetchCars(filterCar: Filter): Observable<CarView[]> {
        ...
        for (const property in filterCar) {
            ...
            if (propertyValue && propertyValue !== 0) params = params.set(property, `${filterCar[property as keyof typeof filterCar]}`)
        }
        console.log(params.toString())

        return this.http.get<CarView[]>(`${environment.restAPI}/car/GetCars`, { params }).pipe(
            first(),
    ...
    validateCarName(carId: number, regNumber: string): Observable<boolean> {
        ...
        return this.http.get<boolean>(`${environment.restAPI}/car/ValidateName`, { params }).pipe(
            first(),
            tap(() => this.loggerService.log('fetch cars')),
    ...
    fetchCar(carId: number): Observable<CarView> {
        return this.http.get<CarView>(`${environment.restAPI}/car/${carId}`).pipe(
            first(),
    ...
    addUpdateCar(car: Car, update: UpdateEnum): Observable<DisplayPartView> {
        ...
        if (update === UpdateEnum.New)
            return this.http.post<DisplayPartView>(`${environment.restAPI}/car`, JSON.stringify(carTemp), this.httpHeader).pipe(
                map((res) => {
            ...
            return this.http.patch<DisplayPartView>(`${environment.restAPI}/car/${car.carId}`, JSON.stringify(carTemp), this.httpHeader).pipe(
                map((res) => {
                    Enrich(res, this.staticSelectionService)
                    return res
                })
    ...
    deleteCar(carId: number): Observable<boolean> {
        return this.http.post<boolean>(`${environment.restAPI}/car/delete`, { id: carId }).pipe(
            first(),
            tap(() => this.loggerService.log(`fetch car: ${carId}`))
        )
    }
    ...
    checkForUniqueness(regName: string, bus: number) {
        ...
        return this.http.get<boolean>(`${environment.restAPI}/car/CheckForUniqueness`, { params }).pipe(
            first(),
